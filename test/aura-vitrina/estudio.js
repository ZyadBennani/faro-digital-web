/* ═══════════════════════════════════════════════════════════════════════════
   estudio.js · EL PLATO DE AURA — sesion 2, /test/aura-vitrina/

   Extraido de test/aura-frasco/frasco.js POR RANGO DE LINEAS, no reescrito:
   1-127 · 185-611 · 740-785. Lo que se ha dejado fuera es SOLO el frasco
   procedural (remuestrear, perfilSolido, perfilLiquido, lathe, perfilTapon,
   construirFrasco), que en la sesion 2 lo sustituyen tres .glb reales.

   El plato NO se toca: es lo que costo seis rondas y tres lecciones
   (L-0066 liquido opaco · L-0067 estudio oscuro · L-0068 dos sombras).
   ═══════════════════════════════════════════════════════════════════════════ */
...
export function svgEtiqueta(def, opciones) {
  ...
  /* 🔴 LA TINTA CONTRASTA CON LO QUE TIENE DETRAS, Y DETRAS ESTA EL PRODUCTO.
     Anadido en la sesion 2. En la sesion 1 la tinta era fija y oscura, y
     funcionaba porque el frasco era procedural y la banda caia donde se
     decidia. Con modelos reales la banda de ALBA cae ENTERA dentro del
     liquido ambar (#6B3208): tinta oscura sobre fondo oscuro, y el nombre se
     leia «AL». No era un fallo de geometria —el arco estaba centrado a 0,2
     grados de la camara— era un fallo de contraste.

         Una etiqueta sobre vidrio no se lee contra el vidrio: se lee contra
         lo que hay al otro lado. Y lo que hay al otro lado es el liquido.

     Es ademas lo que hace un frasco de verdad: serum ambar = impresion clara,
     crema blanca = impresion oscura. */
  const tinta = o.tinta || (impresa ? '#2A1D17' : '#33251F');
...
export function matVidrio(base, cfg) {
...
export function matLiquido(liq, vidrio, cfg) {
  const neg = cfg.material.control_negativo;
...
export function matTapon(t) {
  return new THREE.MeshPhysicalMaterial({
    color: new THREE.Color(t.color),
    roughness: t.roughness,
    metalness: t.metalness,
    clearcoat: 0.0,
    /* 0,60 y no 0,90. Con 0,90 un tapon escrito #332C28 —casi negro— se
       renderizaba gris claro: en un plato oscuro con dos ventanas grandes, la
       parte mate de la escena es la que MAS nota el entorno, porque no lo
       concentra en un brillo, lo reparte por toda la cara. El vidrio puede
       permitirse envMapIntensity 1; el tapon no, o deja de anclar la pieza. */
    envMapIntensity: t.envMapIntensity === undefined ? 0.60 : t.envMapIntensity
  });
}

export function matAnillo(a) {
  return new THREE.MeshPhysicalMaterial({
